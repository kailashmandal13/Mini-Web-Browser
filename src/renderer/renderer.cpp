...
void HTMLRenderer::mousePressEvent(QMouseEvent* event) {
    for (const auto& area : clickableAreas) {
        if (area.rect.contains(event->pos())) {
            QString url = area.url;
            
            // Always emit linkClicked for local files and relative paths
            if (url.startsWith("/") || 
                url.startsWith("./") || 
                url.startsWith("../") ||
                url.startsWith("http://localhost") || 
                url.startsWith("https://localhost") ||
                url.endsWith(".html") ||  // Handle HTML files
                !url.contains("://"))     // Handle relative paths
            {
                qDebug() << "Emitting linkClicked for URL:" << url;
                emit linkClicked(url);
            } else {
                // Only external URLs should open in external browser
